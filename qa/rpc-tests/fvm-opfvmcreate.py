...
class OpCreateTest(BitcoinTestFramework):
    ...
    def basic_contract_is_created_raw_tx_test(self):
        ...
        tx = make_transaction(node,
            [make_vin(node, 500000*FVM_MIN_GAS_PRICE)],
            [make_op_fvmcreate_output(node, 0, 4, CScriptNum(500000), CScriptNum(FVM_MIN_GAS_PRICE), bytes.fromhex("60606040523415600b57fe5b5b60398060196000396000f30060606040525b600b5b5b565b0000a165627a7a72305820e3bed070fd3a81dd00e02efd22d18a3b47b70860155d6063e47e1e2674fc5acb0029"))]
        )
    ...
    def gas_limit_signedness_test(self):
        ...
        tx = make_transaction(node,
            ...
            [make_op_fvmcreate_output(node, 0, b"\x04", b"\xff\xff", 1000, bytes.fromhex("60606040523415600b57fe5b5b60398060196000396000f30060606040525b600b5b5b565b0000a165627a7a7230582092926a9814888ff08700cbd86cf4ff8c50052f5fd894e794570d9551733591d60029"))]
        )
        try:
            node.sendrawtransaction(tx)
            assert(False)
    ...
    def gas_limit_signedness_2_test(self):
        ...
        tx = make_transaction(node,
            ...
            # changing the gas limit \xff\xff -> \xff\xff\x00 results in success.
            [make_op_fvmcreate_output(node, 0, b"\x04", b"\xff\x4f", 1000, bytes.fromhex("60606040523415600b57fe5b5b60398060196000396000f30060606040525b600b5b5b565b0000a165627a7a7230582092926a9814888ff08700cbd86cf4ff8c50052f5fd894e794570d9551733591d60029")),
            make_op_fvmcreate_output(node, 0, b"\x04", b"\xff\xff", 1000, bytes.fromhex("60606040523415600b57fe5b5b60398060196000396000f30060606040525b600b5b5b565b0000a165627a7a7230582092926a9814888ff08700cbd86cf4ff8c50052f5fd894e794570d9551733591d60029"))]
        )
        try:
            node.sendrawtransaction(tx)
