...
class FvmCreateEthOpCodeTest(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def create_contract_with_value_from_contract_test(self):
        ...
        # The total gas is equal to 1 fvm (10^6 * 10^2) + a minor txfee
    ...
    def check_value_transfers_from_and_to_contract_test(self):
        ...
        # Transfer 50 fvm from the contract via p2pkh to an address of our choice
        ...
        # Transfer 50 fvm from the contract via OP_FVMCALL to its parent contract (the Factory contract)
        receiver_address = self.node.getnewaddress()
        h160addr = str(base58_to_byte(receiver_address, 25)[1])[2:-1]
        data = "9e1a00aa"
