...
class FvmSoftMinerGasRelatedLimitsTest(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def send_raw_to_contract(self, node, contract_address, gas_limit, gas_price, num_outputs=1):
        ...
        tx.vout = [CTxOut(amount, scriptPubKey=CScript([b"\x04", CScriptNum(gas_limit), CScriptNum(gas_price), b"\x00", hex_str_to_bytes(contract_address), OP_FVMCALL])) for i in range(num_outputs)]
...
if __name__ == '__main__':
    FvmSoftMinerGasRelatedLimitsTest().main()
