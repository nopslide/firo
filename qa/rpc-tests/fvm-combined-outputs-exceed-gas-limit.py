...
class FvmCombinedOutputsExceedGasLimitTest(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def run_test(self):
        ...
        tx.vout = [
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(1), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_FVMCALL])),
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(1), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_FVMCALL])),
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(1), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_FVMCALL]))
        ]
...
if __name__ == '__main__':
    FvmCombinedOutputsExceedGasLimitTest().main()
