...
FVM_MIN_GAS_PRICE = 40
FVM_MIN_GAS_PRICE_STR = "0.00000040"
...
def make_op_fvmcall_output(value, version, gas_limit, gas_price, data, contract):
...
def make_op_fvmcreate_output(node, value, version, gas_limit, gas_price, data):
    scriptPubKey = CScript()
    scriptPubKey += version
    scriptPubKey += gas_limit
    scriptPubKey += gas_price
    scriptPubKey += data
    scriptPubKey += OP_FVMCREATE
    return CTxOut(value, scriptPubKey)
