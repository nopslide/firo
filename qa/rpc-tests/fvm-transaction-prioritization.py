...
class FvmTransactionPrioritizationTest(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def send_op_call_transaction_with_gas_price(self, contract_address, gas_price, spends_txid=None, spends_vout=None):
        ...
        tx.vout.append(CTxOut(0, scriptPubKey=CScript([b"\x04", CScriptNum(gas_limit), CScriptNum(int(gas_price*COIN)), b"\x00", hex_str_to_bytes(contract_address), OP_FVMCALL])))
    ...
    def send_op_fvmcall_outputs_with_gas_price(self, contract_address, gas_prices, spends_txid=None, spends_vout=None):
        gas_limit = 100000
        if not spends_txid:
            for unspent in self.node.listunspent():
    ...
    def verify_contract_txs_internal_order_test(self, with_restart=False, use_staking=False):
        ...
        tx4 = self.send_op_fvmcall_outputs_with_gas_price(contract_address, [0.0001])
        tx5 = self.send_op_fvmcall_outputs_with_gas_price(contract_address, [0.0001, 0.0001])
        tx3 = self.send_op_fvmcall_outputs_with_gas_price(contract_address, [0.00010001])
        tx6 = self.send_op_fvmcall_outputs_with_gas_price(contract_address, [0.0001, 0.00010001, 0.00010001])
        tx2 = self.send_op_fvmcall_outputs_with_gas_price(contract_address, [0.002])
    ...
    def verify_contract_ancestor_txs_test(self, with_restart=False, use_staking=False):
        ...
        for (expected_tx_index, gas_price) in [(1, 60), (2, 50), (7, 40), (8, 50)]:
            ...
            tx.vin = [CTxIn(COutPoint(int(unspent['txid'], 16), unspent['vout']), nSequence=0)]
            tx.vout = [
                CTxOut(0, scriptPubKey=CScript([b"\x04", CScriptNum(30000), CScriptNum(gas_price), b"\x00", hex_str_to_bytes(contract_address), OP_FVMCALL])),
...
if __name__ == '__main__':
    FvmTransactionPrioritizationTest().main()
