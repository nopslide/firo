...
struct CompareModifiedEntry {
    ...
    {
        bool fAHasOpFVMCreateOrCall = a.iter->GetTx().HasOpFVMCreateOrCall();
        bool fBHasOpFVMCreateOrCall = b.iter->GetTx().HasOpFVMCreateOrCall();

        // If either of the two entries that we are comparing has a contract scriptPubKey, the comparison here takes precedence
        if(fAHasOpFVMCreateOrCall || fBHasOpFVMCreateOrCall) {

            // Prioritze non-contract txs
            if(fAHasOpFVMCreateOrCall != fBHasOpFVMCreateOrCall) {
                return fAHasOpFVMCreateOrCall ? false : true;
            }
...
public:
    ...
    std::unique_ptr<CBlockTemplate> CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx=true, int64_t* pTotalFees = 0, int32_t nTimeLimit  = 0);
