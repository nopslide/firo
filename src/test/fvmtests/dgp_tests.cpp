...
void contractLoading(){
    ...
    dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::FvmMainNetwork, 1400)));
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test3){
    ...
    contractLoading();
    FvmDGP fvmDGP(globalState.get());
    dev::eth::EVMSchedule schedule = fvmDGP.getGasSchedule(1400);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    FvmDGP fvmDGP(globalState.get());
    dev::eth::EVMSchedule schedule = fvmDGP.getGasSchedule(502); // After initializing the tests, the height of the chain 502
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    contractLoading();
    FvmDGP fvmDGP(globalState.get());
    uint32_t blockSize = fvmDGP.getBlockSize(100);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    FvmDGP fvmDGP(globalState.get());
    uint32_t blockSize = fvmDGP.getBlockSize(502);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    FvmDGP fvmDGP(globalState.get());
    uint64_t minGasPrice = fvmDGP.getMinGasPrice(502);
