...
ResultExecute FvmState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, FvmTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){
    ...
    if(voutLimit){
        ...
        if(_t.value() > 0) {
            refund.vin.push_back(CTxIn(h256Touint(_t.getHashWith()), _t.getNVout(), CScript() << OP_FVMSPEND));
...
dev::Address FvmState::createFvmAddress(dev::h256 hashTx, uint32_t voutNumber){
...
std::vector<CTxIn> CondensingTX::createVins(){
    ...
    for(auto& v : vins){
        if((v.second.value > 0 && v.second.alive) || (v.second.value > 0 && !vins[v.first].alive && !checkDeleteAddress(v.first)))
            ins.push_back(CTxIn(h256Touint(v.second.hash), v.second.nVout, CScript() << OP_FVMSPEND));
    }
    return ins;
}
    ...
    for(auto& b : balances){
        if(b.second > 0){
            ...
            if(a && a->isAlive()){
                ...
                script = CScript() << valtype{0} << valtype{0} << valtype{0} << valtype{0} << b.first.asBytes() << OP_FVMCALL;
            } else {
